...
class MomentDataset(Dataset):
    ...
    def clip4cap_get_text(self, caption=None):
        k = 1
        pairs_text = np.zeros((k, self.args.max_words), dtype=np.int64)

        pairs_input_caption_ids = np.zeros((k, self.args.max_words), dtype=np.int64)
        pairs_output_caption_ids = np.zeros((k, self.args.max_words), dtype=np.int64)
        pairs_decoder_mask = np.zeros((k, self.args.max_words), dtype=np.int64)
